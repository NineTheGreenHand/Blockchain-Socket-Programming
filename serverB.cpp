// serverB.cpp
// Back-ServerB file for the socket programming project
...
#define BPORT "22687"           // UDP port number used by ServerB
#define MPORT "24687"           // UDP port number used by ServerM
#define LOGFILE "block2.txt"    // File that is accessible by ServerB
...
vector<int> read_File(char *targetUser) {
    ...
    while (getline(file, line)) {
        ...
        char seqB[words[0].length() + 1];
        strcpy(seqB, words[0].c_str());
        char coinB[words[3].length() + 1];
        strcpy(coinB, words[3].c_str());

        seq = max(seq, atoi(seqB));
...
int main(void) {
    ...
    // Get backend server information, we use HOSTADDRESS and BPORT in here:
    if ((rv = getaddrinfo(HOSTADDRESS, BPORT, &hints, &servinfo)) != 0) {
