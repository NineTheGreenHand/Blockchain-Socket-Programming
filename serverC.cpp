...
#define CPORT "23687"           // UDP port number used by ServerC
...
vector<int> read_File(char *targetUser) {
    ...
    while (getline(file, line)) {
        ...
        char seqC[words[0].length() + 1];
        strcpy(seqC, words[0].c_str());
        char coinC[words[3].length() + 1];
        strcpy(coinC, words[3].c_str());

        seq = max(seq, atoi(seqC));
...
int main(void) {
    ...
    // Get backend server information, we use HOSTADDRESS and CPORT in here:
    if ((rv = getaddrinfo(HOSTADDRESS, CPORT, &hints, &servinfo)) != 0) {
