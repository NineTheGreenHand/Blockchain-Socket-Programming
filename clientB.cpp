...
// clientB.cpp
// Client B file for the socket programming project
...
int main(int argc, char *argv[]) {
    ...
    serv_addr.sin_port = htons(26687);
    ...
    if ((numbytes = send(sockfd, message, strlen(message), 0)) == -1) {
        perror("Client B: failed to send message to Server M.");
        exit(1);
    }
    ...
    if ((numbytes = recv(sockfd, buf, MAXDATASIZE - 1, 0)) == -1) {
        perror("Client B: failed to receive message from Server M.");
        exit(1);
    }
